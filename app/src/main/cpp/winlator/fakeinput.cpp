...
static int (*my_ppoll)(struct pollfd *, nfds_t, const struct timespec *, const sigset_t *) = nullptr;
static int (*my_select)(int, fd_set *, fd_set *, fd_set *, struct timeval *) = nullptr;
...
static void send_vibration(int strong, int weak, uint16_t duration_ms, uint16_t slot) {
...
__attribute__((visibility("hidden")))
static bool is_fake_input_node_path(const char *pathname) {
    return pathname &&
           (!strncmp(pathname, "/dev/input/event", 16) ||
...
__attribute__((visibility("hidden")))
static void copy_ioctl_string(int op, void *argp, const char *value) {
...
__attribute__((visibility("hidden")))
static bool is_fake_input_fd(int fd) {
...
__attribute__((visibility("hidden")))
static long long timespec_to_ms(const struct timespec *timeout) {
    if (!timeout)
        return -1;
...
__attribute__((visibility("hidden")))
static long long timeval_to_ms(const struct timeval *timeout) {
    if (!timeout)
        return -1;
    return static_cast<long long>(timeout->tv_sec) * 1000LL +
...
static long long monotonic_ms() {
    ...
    return static_cast<long long>(now.tv_sec) * 1000LL +
...
EXPORT int ioctl(int fd, int op, ...) {
    ...
    if (type == 0x45 && number == 0x1) {
        ...
        if (effect->type == FF_RUMBLE) {
            ...
            send_vibration(effect->u.periodic.magnitude,
...
EXPORT ssize_t write(int fd, const void *buf, size_t count) {
    ...
    auto controller = controller_map.find(fd);
    if (controller != controller_map.end() && count == sizeof(struct input_event)) {
    ...
    if (controller != controller_map.end()) {
        ...
        for (int i = 0; i < iovcnt; i++) {
            if (iov[i].iov_len == sizeof(struct input_event)) {
                check_ff_event(static_cast<const struct input_event *>(iov[i].iov_base), slot);
            }
        }
    }
...
EXPORT int poll(struct pollfd *fds, nfds_t nfds, int timeout) {
    ...
    while (true) {
        ...
        for (nfds_t i = 0; i < nfds; i++) {
            ...
            short revents = 0;
            if ((fds[i].events & (POLLIN | POLLRDNORM)) && fake_fd_has_unread_data(fds[i].fd))
...
EXPORT int ppoll(struct pollfd *fds, nfds_t nfds, const struct timespec *timeout, const sigset_t *sigmask) {
    if (!my_ppoll)
        *(void **)&my_ppoll = dlsym(RTLD_NEXT, "ppoll");

    if (sigmask)
        return my_ppoll ? my_ppoll(fds, nfds, timeout, sigmask) : syscall(SYS_ppoll, fds, nfds, timeout, sigmask, sizeof(sigset_t));

    return poll(fds, nfds, static_cast<int>(timespec_to_ms(timeout)));
}

EXPORT int select(int nfds, fd_set *readfds, fd_set *writefds, fd_set *exceptfds, struct timeval *timeout) {
    if (!my_select)
    ...
    if (!has_fake_fds)
        return my_select ? my_select(nfds, readfds, writefds, exceptfds, timeout) : -1;

    const long long timeout_ms = timeval_to_ms(timeout);
    const long long deadline_ms = timeout_ms < 0 ? -1 : monotonic_ms() + timeout_ms;
    ...
    while (true) {
        ...
        int real_ready = my_select
                ? my_select(nfds,
                            readfds ? &iter_readfds : nullptr,
                            writefds ? &iter_writefds : nullptr,
                            exceptfds ? &iter_exceptfds : nullptr,
                            &zero_timeout)
                : 0;
        ...
        for (int fd = 0; fd < nfds; fd++) {
            if (!is_fake_input_fd(fd))
                continue;
            if (readfds && FD_ISSET(fd, &original_readfds) && fake_fd_has_unread_data(fd)) {
                FD_SET(fd, readfds);
                ready++;
            }
        }
