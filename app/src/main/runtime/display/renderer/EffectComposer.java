...
public class EffectComposer {
  ...
  private void initBuffers() {
    ...
    if (readBuffer == null) {
      ...
      //            Log.d(TAG, "Initialized readBuffer with size: " + renderer.getSurfaceWidth() +
      // "x" + renderer.getSurfaceHeight());
    }

    if (writeBuffer == null) {
      writeBuffer = new RenderTarget();
      writeBuffer.allocateFramebuffer(renderer.getSurfaceWidth(), renderer.getSurfaceHeight());
      //            Log.d(TAG, "Initialized writeBuffer with size: " + renderer.getSurfaceWidth() +
      // "x" + renderer.getSurfaceHeight());
    }
  }
  ...
  public synchronized void addEffect(Effect effect) {
    ...
    // Move this call to the end of a batch effect addition or modification to prevent immediate
    // rendering
  ...
  public synchronized void removeEffect(Effect effect) {
    if (effects.remove(effect)) {
      ...
      //            Log.d(TAG, "Effect not found for removal: " +
  ...
  public synchronized void render() {
    ...
    for (Effect effect : effects) {
      ...
      //            Log.d(TAG, "Binding to " + (renderToScreen ? "screen" : "writeBuffer") + "
      // framebuffer: " + targetFramebuffer);

      GLES20.glViewport(0, 0, renderer.surfaceWidth, renderer.surfaceHeight);
      renderer.setViewportNeedsUpdate(true);
      //            Log.d(TAG, "Viewport updated to size: " + renderer.surfaceWidth + "x" +
  ...
  private void renderEffect(Effect effect) {
    ...
    //        Log.d(TAG, "Uniforms set: resolution=" + renderer.surfaceWidth + "x" +
    // renderer.surfaceHeight + ", screenTexture=" + readBuffer.getTextureId());
