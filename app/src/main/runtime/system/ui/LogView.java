package com.winlator.cmod.runtime.system.ui;
...
public class LogView extends View {
  ...
  private void drawScrollThumbs(Canvas canvas) {
    ...
    canvas.drawRoundRect(
        scrollThumbX,
        getHeight() - minScrollThumbSize,
        scrollThumbX + scrollThumbWidth,
        getHeight(),
        radius,
        radius,
        paint);
    canvas.drawRoundRect(
        getWidth() - minScrollThumbSize,
        scrollThumbY,
        getWidth(),
  ...
  public float getScrollThumbY() {
    ...
    if (scrollSize.y > 0 && scrollSize.y > height)
      return scrollPosition.y * (height / scrollSize.y);
    return -Float.MAX_VALUE;
  }
  ...
  private void computeScrollSize() {
    ...
    paint.setTextSize(defaultTextSize);
    for (int i = 0, count = lines.size(); i < count; i++)
      maxWidth = Math.max(paint.measureText(lines.get(i)), maxWidth);
