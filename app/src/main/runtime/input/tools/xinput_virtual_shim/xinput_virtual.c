...
#define DBG(...)                                                               \
  do {                                                                         \
    fprintf(stderr, "XINPUT_VIRTUAL: " __VA_ARGS__);                           \
    fflush(stderr);                                                            \
  } while (0)
#ifndef ERROR_EMPTY
...
static BOOL build_slot_path(DWORD user_index, char *path, size_t path_len) {
  ...
  DWORD rc =
      GetEnvironmentVariableA("EVSHIM_WIN_PATH", base, (DWORD)sizeof(base));
  if (rc == 0) {
  ...
  if (path_len == 0 || rc == 0 || rc >= sizeof(base) ||
  ...
  if (suffix != NULL) {
    ...
    _snprintf(path, path_len, "%s\\gamepad%lu.mem", normalized,
...
static BOOL ensure_slot(DWORD user_index) {
  ...
  slot->mapping = CreateFileMappingA(slot->file, NULL, PAGE_READWRITE, 0,
                                     sizeof(gamepad_io), NULL);
  if (!slot->mapping) {
  ...
  slot->view = (volatile gamepad_io *)MapViewOfFile(
      slot->mapping, FILE_MAP_READ | FILE_MAP_WRITE, 0, 0, sizeof(gamepad_io));
  if (!slot->view) {
...
static DWORD fill_state(DWORD user_index, XINPUT_STATE *state) {
  ...
  if (memcmp(&fresh, slot->last_packet, sizeof(slot->last_packet)) != 0) {
    ...
    DBG("P%lu STATE CHANGE: lx=%d ly=%d rx=%d ry=%d lt=%d rt=%d pkt=%lu\n",
        (unsigned long)user_index, (int)raw->lx, (int)raw->ly, (int)raw->rx,
        (int)raw->ry, (int)raw->lt, (int)raw->rt,
        (unsigned long)slot->packet_number);
  }

  if (++g_fill_dbg_count % 500 == 1) {
    DBG("P%lu POLL #%d: lx=%d ly=%d rx=%d ry=%d\n", (unsigned long)user_index,
        g_fill_dbg_count, (int)raw->lx, (int)raw->ly, (int)raw->rx,
        (int)raw->ry);
  }
...
__declspec(dllexport) DWORD WINAPI XInputGetState(DWORD user_index,
                                                  XINPUT_STATE *state) {
  return fill_state(user_index, state);
}

__declspec(dllexport) DWORD WINAPI XInputGetStateEx(DWORD user_index,
                                                    XINPUT_STATE *state) {
  return fill_state(user_index, state);
}

__declspec(dllexport) DWORD WINAPI XInputSetState(DWORD user_index,
                                                  XINPUT_VIBRATION *vibration) {
  slot_state *slot;
...
__declspec(dllexport) DWORD WINAPI XInputGetCapabilities(
    DWORD user_index, DWORD flags, XINPUT_CAPABILITIES *caps) {
  if (!caps) {
    return ERROR_BAD_ARGUMENTS;
  }
...
__declspec(dllexport) void WINAPI XInputEnable(BOOL enable) { (void)enable; }

__declspec(dllexport) DWORD WINAPI XInputGetBatteryInformation(
    DWORD user_index, BYTE dev_type, XINPUT_BATTERY_INFORMATION *battery) {
...
__declspec(dllexport) DWORD WINAPI XInputGetKeystroke(
    DWORD user_index, DWORD reserved, XINPUT_KEYSTROKE *keystroke) {
  (void)reserved;
  if (keystroke) {
...
__declspec(dllexport) DWORD WINAPI XInputGetAudioDeviceIds(
    DWORD user_index, LPWSTR render_device_id, UINT *render_count,
...
__declspec(dllexport) DWORD WINAPI XInputWaitForGuideButton(DWORD user_index,
                                                            DWORD flags,
                                                            void *unknown) {
  (void)user_index;
  (void)flags;
...
XInputCancelGuideButtonWait(DWORD user_index) {
  (void)user_index;
  return ERROR_CALL_NOT_IMPLEMENTED;
}
...
__declspec(dllexport) DWORD WINAPI XInputGetBaseBusInformation(DWORD user_index,
                                                               void *info) {
  (void)user_index;
  (void)info;
  return ERROR_CALL_NOT_IMPLEMENTED;
